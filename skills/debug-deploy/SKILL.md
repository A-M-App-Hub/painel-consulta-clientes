...
# Debug Deploy — `painel consulta clientes`
...
- Espelho read-only **se** o usuário fornecer `A-M-App-Hub/painel-consulta-clientes` (registrar repo no ADR, sem persistir em assets)
...
5. Pergunta: *Tem deploy de referência similar? `A-M-App-Hub/painel-consulta-clientes` ou `skip`.* (mesmo contrato das outras skills)
