...
metadata:
  persona: Sentinela
  repo_name: "painel-consulta-clientes"
  service_name: "painelconsultaclient"
  topology: "FastAPI_Mixed + INTERNAL_CAS"
  project_display_name: "painel consulta clientes"
...
# Security Pipeline — `painel consulta clientes`
...
```bash
gh secret list --repo painel-consulta-clientes | grep -q 'GCP_WORKLOAD_IDENTITY_PROVIDER' || \
  echo "AVISO: GCP_WORKLOAD_IDENTITY_PROVIDER não encontrado em secrets"
gh secret list --repo painel-consulta-clientes | grep -q 'GCP_SERVICE_ACCOUNT' || \
  echo "AVISO: GCP_SERVICE_ACCOUNT não encontrado em secrets"
gh variable list --repo painel-consulta-clientes | grep -q 'PROJECT_ID' || \
...
```bash
gh workflow run security-cloud-audit.yml --repo painel-consulta-clientes -f dry_run=true
sleep 30
...
```bash
gh workflow run security-post-qa.yml --repo A-M-App-Hub/painel-consulta-clientes
...
```bash
gh workflow run security-cloud-audit.yml --repo A-M-App-Hub/painel-consulta-clientes
...
gh run download "$B1_RUN_ID" \
  --repo A-M-App-Hub/painel-consulta-clientes \
...
# Security Report — painel consulta clientes
