...
# Bootstrap Deploy — `painel consulta clientes`
...
- Repositório espelho apenas se o **usuário** informar `A-M-App-Hub/painel-consulta-clientes` (read-only via `gh api`), registrado no ADR
...
5. Pergunta ao usuário: *Deploy bem-sucedido similar? `A-M-App-Hub/painel-consulta-clientes` ou `skip`.*
